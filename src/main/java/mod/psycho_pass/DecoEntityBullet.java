...
public class DecoEntityBullet extends Entity implements IProjectile, IThrowableEntity {
	private int xTile = -1;
	private int yTile = -1;
	private int zTile = -1;
	private int inTile;
	...
	private double damage = 3F;
	private int knockbackStrength;
	private double bom1;
	private EntityPlayer EntityPlayer1;
	private double bom2;
	private double bom3;
	private World bomworld;
	...
	public DecoEntityBullet(World par1World) {
		super(par1World);
		this.renderDistanceWeight = 10.0D;
		this.setSize(0.5F, 0.5F);
	}

	public DecoEntityBullet(EntityPlayer par2EntityPlayer, World par1World, double par2, double par4, double par6) {
		super(par1World);
		this.renderDistanceWeight = 10.0D;
		this.setSize(0.5F, 0.5F);
	...
	public DecoEntityBullet(World par1World, EntityLivingBase par2EntityLivingBase,
		...
		if (d3 >= 1.0E-7D) {
			...
			double d4 = d0 / d3;
			double d5 = d2 / d3;
			this.setLocationAndAngles(par2EntityLivingBase.posX + d4, this.posY, par2EntityLivingBase.posZ + d5, f2,
					f3);
			this.yOffset = 0.0F;
	...
	public DecoEntityBullet(World par1World, EntityLivingBase par2EntityLivingBase, float par3) {
		super(par1World);
		this.renderDistanceWeight = 10.0D;
		this.shootingEntity = par2EntityLivingBase;
	...
	public void onUpdate() {
		...
		if (i > 0) {
			...
			Block.getBlockById(i).setBlockBoundsBasedOnState(this.worldObj, this.xTile, this.yTile, this.zTile);
			AxisAlignedBB axisalignedbb = Block.getBlockById(i).getCollisionBoundingBoxFromPool(this.worldObj,
		...
		if (this.inGround) {
			...
			if (movingobjectposition != null) {
				if (movingobjectposition.entityHit != null) {
					...
					Explosion ex = new Explosion(worldObj, this, x, y, z, size);
					ex.isSmoking = smoke;
					ex.isFlaming = particle;
					ex.doExplosionA();
					ex.doExplosionB(particle);
					...
					int i11 = MathHelper.ceiling_double_int((double) f2 * this.damage);

					if (this.getIsCritical()) {
						i11 += this.rand.nextInt(i11 / 2 + 2);
					}

					DamageSource damagesource = null;

					if (this.shootingEntity == null) {
						damagesource = Psycho_pass_core.causeTestDecoDamage(this, this);
					} else {
						damagesource = Psycho_pass_core.causeTestDecoDamage(this, this.shootingEntity);
					}
					...
					if (movingobjectposition.entityHit.attackEntityFrom(damagesource, (float) i11)) {
						if (movingobjectposition.entityHit instanceof EntityLivingBase) {
							EntityLivingBase entitylivingbase = (EntityLivingBase) movingobjectposition.entityHit;
                            entitylivingbase.setHealth(0);//対象のハートを０に
					...
					} else if (!(movingobjectposition.entityHit instanceof EntityPlayer)) // これでPlayerに当たった場合は以下の処理を行わないようにする
					{
						/*
						 * this.motionX *= -0.10000000149011612D; this.motionY
						 * *= -0.10000000149011612D; this.motionZ *=
						 * -0.10000000149011612D; this.rotationYaw += 180.0F;
					...
					if (this.inTile != 0) {
						Block.getBlockById(this.inTile).onEntityCollidedWithBlock(this.worldObj, this.xTile, this.yTile,
								this.zTile, this);
					}
				}
			}
