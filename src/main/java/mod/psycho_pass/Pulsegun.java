...
public class Pulsegun extends Item {
	...
	public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer) {
		...
		try {
			...
			Psycho_pass_core.output("----------------------------------");
		}catch(IndexOutOfBoundsException  ex){//例外！
			Psycho_pass_core.output(ex+"");
			ex.printStackTrace();
			par1ItemStack.getTagCompound().setBoolean("SibylSystem", false);
		}
				...
				if (fast == 1) {		// 初期起動か判定
					...
					if (!par3EntityPlayer.capabilities.isCreativeMode){//サバイバル時のみ使用可
						...
						if  (par3EntityPlayer.inventory.armorItemInSlot(2) == null)//Armorslotのチェストが空の場合
						{
							...
							getitem = par3EntityPlayer.inventory.armorItemInSlot(2).getItem();//空じゃない場合  入ってるアイテムを挿入
						}
						...
						if (itemname.toString().equals("MobileTerminal")|| par1ItemStack.getTagCompound().getByte("tagmode") == 4)//チェストにIDが入ってたら
						{
							...
							if (tagUUID.equals(nowUUID)|| !Psycho_pass_core.usercheck){//UUIDとプレイヤー名を比較
								...
								if (par1ItemStack.getItemDamage() == this.maxdamage) {
									// シュビラがあれば消費して耐久値回復
									if (par3EntityPlayer.inventory.hasItem(Psycho_pass_core.pulsegunBullet)||par1ItemStack.getTagCompound().getBoolean("SibylSystem")) {
										if (par1ItemStack.getTagCompound().getBoolean("iderror")){               //NBTがIDエラーかどうかをcheck
											...
											if (par1ItemStack.getTagCompound().getBoolean("ofline")){            //NBTがオンラインかどうかをcheck
												par1ItemStack.getTagCompound().setBoolean("system", true);      //通信エラーを初めてに変更
												if (Psycho_pass_core.games == 1)                                 // モードによって動作を切り替え 自動モード
												{
													switch (mode){
													case 1 :
														par2World.playSoundAtEntity(par3EntityPlayer,"psycho_pass:check", 5.0F, 1.0F);
														par1ItemStack.getTagCompound().setByte("tagmode", (byte) 0);
														mode = 2;
														break;
													case 2 :
														par2World.playSoundAtEntity(par3EntityPlayer,"psycho_pass:check1", 5.0F, 1.0F);
														par1ItemStack.getTagCompound().setByte("tagmode", (byte) 0);
														mode = 3;
														break;
													case 3 :
														par2World.playSoundAtEntity(par3EntityPlayer,"psycho_pass:check2", 5.0F, 1.0F);
														par1ItemStack.getTagCompound().setByte("tagmode", (byte) 0);
														mode = 4;
														break;
													case 4 :
														par2World.playSoundAtEntity(par3EntityPlayer,"psycho_pass:check3", 5.0F, 1.0F);
														par1ItemStack.getTagCompound().setByte("tagmode", (byte) 0);
														mode = 5;
														break;
													case 5 :
														par2World.playSoundAtEntity(par3EntityPlayer,"psycho_pass:check4", 5.0F, 1.0F);
														par1ItemStack.getTagCompound().setByte("tagmode", (byte) 1);
														mode = 6;
														break;
													case 6 :
														par2World.playSoundAtEntity(par3EntityPlayer,"psycho_pass:check5", 5.0F, 1.0F);
														par1ItemStack.getTagCompound().setByte("tagmode", (byte) 2);
														mode = 1;
														break;
													}
												} else {// マニュアルモード
													if (Psycho_pass_core.r == 1) // rが1のとき下記の測定音を再生 パラライザー
													{
														switch (b){
														case 1 :
															par2World.playSoundAtEntity(par3EntityPlayer,"psycho_pass:check", 5.0F, 1.0F);
													...
													if (Psycho_pass_core.r == 2) // rが2のとき下記の測定音を再生 エリミネーター
													{
														par2World.playSoundAtEntity(par3EntityPlayer,"psycho_pass:check4", 5.0F, 1.0F);
														par1ItemStack.getTagCompound().setByte("tagmode", (byte) 1);
													}
													if (Psycho_pass_core.r == 3) // rが3のとき下記の測定音を再生 でコンポーザー
													{
														par2World.playSoundAtEntity(par3EntityPlayer,"psycho_pass:check5", 5.0F, 1.0F);
														par1ItemStack.getTagCompound().setByte("tagmode", (byte) 2);
													}
													...
													{
														par2World.playSoundAtEntity(par3EntityPlayer,"psycho_pass:special", 5.0F, 1.0F);
														par1ItemStack.getTagCompound().setByte("tagmode", (byte) 4);
													}
												}
												par3EntityPlayer.inventory.consumeInventoryItem(Psycho_pass_core.pulsegunBullet);// シュビラ消費
												par1ItemStack.damageItem(this.maxdamage * -1, par3EntityPlayer);/* 耐久値回復 */

											}else{//通信エラー後インベントリにシビュラシステムがあったら以下の処理
												par2World.playSoundAtEntity(par3EntityPlayer,"psycho_pass:systemonline", 5.0F, 1.0F);//オンラインを再生
											...
											par2World.playSoundAtEntity(par3EntityPlayer,"psycho_pass:"+lowname, 5.0F, 1.0F);//ユーザー認証を再生
											par1ItemStack.getTagCompound().setByte("tagmode", (byte) 0);
										...
										if (!par1ItemStack.getTagCompound().getBoolean("system")){       //初めての通信エラーかcheck
											par2World.playSoundAtEntity(par3EntityPlayer,"psycho_pass:error", 1.0F, 1.0F); // シュビラが手持ちアイテム内にないときに通信エラーを再生
									...
									//if (par3EntityPlayer.getLastAttackerTime() == 0) // 連射防止
									//{
									if (!par1ItemStack.getTagCompound().getString("before").equals(getitem.toString())){//もし、IDが変わっていたら
										par1ItemStack.getTagCompound().setBoolean("iderror",false);                     //再度ユーザー認証するように
									}
									int gettagmode = par1ItemStack.getTagCompound().getByte("tagmode");
									ParaEntityBullet paraentitybullet = new ParaEntityBullet(par2World, par3EntityPlayer,this.range * 2.0F);
									EriEntityBullet erientitybullet = new EriEntityBullet(par2World, par3EntityPlayer,this.range * 2.0F);
									DecoEntityBullet decoentitybullet = new DecoEntityBullet(par2World, par3EntityPlayer,this.range * 2.0F);
									String soundtype = "";
									...
									if (gettagmode != 4|| !par1ItemStack.getTagCompound().getBoolean("SibylSystem")){//常時解除モードigaiのNBTが書き込まれてたら もしくはオフライン
							...
							if (par1ItemStack.getTagCompound().getBoolean("idsystem")){//初めてのIDエラーかcheck
								...
								par1ItemStack.getTagCompound().setBoolean("idsystem", false);//初めてを解除
							}else{
								par2World.playSoundAtEntity(par3EntityPlayer,"psycho_pass:lock", 5.0F, 1.0F);//2回目以降は空打ち音
							}
							if (par1ItemStack.getItemDamage() == 0 && !par3EntityPlayer.capabilities.isCreativeMode){//もしドミネーターが打てる状態だったら
								//par3EntityPlayer.inventory.addItemStackToInventory(new ItemStack(Psycho_pass_core.pulsegunBullet));  //アイテム「シビュラシステム」を返却
